...
const Navigation = ({ paddingB, background }) => {
  ...
  const { isDark, setIsDark,totalItem,rentalLink } = useGlobalContext(AppContext);
      ...
      <div className=" ">
        <div className=" flex  items-center  justify-between gap-6">
          ...
          <div className="flex items-center gap-6 justify-end w-48 md:mr:0 mr-12">
           ...
           <div className="relative" onClick={()=>(navigateLink(rentalLink))}>
            <TbBrandBooking className="text-orange-900 w-8 h-8 cursor-pointer" />
            <span
              className="absolute bg-red-500 rounded-full w-5 h-5 text-sm flex items-center justify-center -top-2 -right-1 text-white-green"
              id="total_items"
            >
              {totalItem}
            </span>
